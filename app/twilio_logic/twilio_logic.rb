...
class TwilioLogic
  ...
  def send_alert_general_message(from_number)
    ...
    if recruit == nil
      ...
      send_alert_general_message(from_number) unless Rails.env.test?
    else
      Message.create(body: message_body, recruit_id: recruit.id, from_recruit: true)
      send_alert_message(recruit) unless Rails.env.test?
    end
